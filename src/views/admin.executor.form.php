<form id="admin-executor-form" class="user-form" method="post" onsubmit="return false;"
      data-api-action="{{? it.editMode }}update-executor{{?}}{{? !it.editMode }}add-executor{{?}}"
      data-after-save-go-to="{{= it.backUrl }}">
    {{? it.editMode }}
        <h1><?php echo \Dictionary\translate('Executor account editing'); ?></h1>
    {{?}}
    {{? !it.editMode }}
        <h1><?php echo \Dictionary\translate('Executor account creation'); ?></h1>
    {{?}}
    <div class="inputs">
        {{? it.editMode }}
            <input name="id" type="hidden" value="{{=it.item.id}}" id="user-id">
        {{?}}
        ...
        <div class="form-group">
            ...
            <div class="input-group">
                <input name="email" type="email" value="{{=it.item.email || ''}}"
                       required autofocus  {{? it.editMode }}disabled{{?}}
