<div class="row data-grid-header">
    <div class="col-md-9">
        <h1><?php echo \Dictionary\translate('Executors'); ?></h1>
    </div>
    <div class="col-md-3 actions">
        <a href="?route=executor/executor-add" class="create-link btn btn-primary btn-sm">
            <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
            <?php echo \Dictionary\translate('Add executor'); ?>
        </a>
    </div>
</div>
...
<table class="table table-striped table-hover">
    <thead>
        <tr>
            ...
            <th><?php echo \Dictionary\translate('Balance'); ?></th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="actions">
                <a data-route="executor/executor-edit" data-params="id={{=item.id}}"
                   class="edit-link btn btn-primary btn-sm" href="javascript:void(0)">
                    <span class="glyphicon glyphicon-edit" aria-hidden="true"></span>
                    <?php echo \Dictionary\translate('Edit'); ?>
                </a>
                {{? item.is_active == 1 }}
                    <a data-api-action="update-executor" data-params="id={{=item.id}}&is_active=0" data-method="post"
                ...
                {{?}}
                {{? item.is_active == 0 }}
                    <a data-api-action="update-executor" data-params="id={{=item.id}}&is_active=1" data-method="post"
